...
export function useSalePayments() {
    ...
    const salePaymentFormData = useForm({
        defaultData: { id: '', sale_order_id: '', method: 'EFECTIVO', amount: '', date: new Date(Date.now()) },
    ...
    async function handleSubmit(e: any) {
        ...
        if (validate()) {
            try {
                if (showForm === 'NEW') {
                    await db.sale_payments.add({ ...formData, id: undefined });
                    setBodyMessage('Pago guardado correctamente.');
                } else if (showForm === 'EDIT') {
                    await db.sale_payments.update(formData.id, formData);
